...
size_t img_size;
uint16_t chunk_size;
volatile uint8_t ili9341_dma_busy = 0;
...
static void ILI9341_WriteImgData(uint8_t* buff, size_t buff_size) {
...
//void ILI9341_DrawImage8_DMA(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint8_t* data)
//{
//	if ((x >= ILI9341_WIDTH) || (y >= ILI9341_HEIGHT)) return;
//	if ((x + w - 1) >= ILI9341_WIDTH) return;
//	if ((y + h - 1) >= ILI9341_HEIGHT) return;
...
//	ILI9341_Select();
//	ILI9341_SetAddressWindow(x, y, x + w - 1, y + h - 1);
//	ILI9341_WriteCommand(0x2C); // RAM write command
//	ILI9341_WriteImgData((uint8_t*)data, w * h * 2); // DMA로 전송 시작 (16bpp 기준)
//	// ILI9341_Unselect()는 DMA 완료 콜백에서 처리해야 함!
//}
