/**
 * OV5647.h
 *  - OV5647 I2C/MIPI Camera Sensor Control
 *  - ChatGPT, 2024.07
 */
#ifndef OV5647_H_
#define OV5647_H_
...
#include "../hdmi/VideoOutput.h"   // 해상도 enum, 타이밍
...
namespace OV5647_cfg {
    ...
    // 모드(enum은 VideoOutput_OV5647.h의 OV5647_Resolution과 일치시켜야 함)
    using mode_t = enum {
        MODE_2592_1944_15 = 0,  // 5MP
        MODE_1080P_1920_1080_30,
        MODE_VGA_640_480_60,
        MODE_END
    };
    using config_modes_t = struct { mode_t mode; config_word_t const* cfg; size_t cfg_size; };

    using awb_t = enum { AWB_DISABLED = 0, AWB_SIMPLE, AWB_END };
    using config_awb_t = struct { awb_t awb; config_word_t const* cfg; size_t cfg_size; };

    using test_t = enum { TEST_DISABLED = 0, TEST_COLORBAR, TEST_END };

    // 주요 레지스터
    uint16_t const OV5647_REG_COLORBAR = 0x503D;
    uint16_t const REG_SW_RESET = 0x0103;
    uint16_t const REG_SW_STREAM = 0x0100;

    config_word_t const cfg_2592_1944_15[] = {
    		{0x0100, 0x00},
			{0x0103, 0x01},
			{0x3034, 0x1a},
			{0x3035, 0x21},
			...
			//			 --- 밝기 증가용 (Exposure 증가) ---
			{0x3500, 0x01},
			{0x3501, 0x20}, // 기본값보다 더 높이면 더 밝음
			{0x3502, 0x00},
			{0x0100, 0x01},
    };

    config_word_t const cfg_1080p_30[] = {
    		{0x0100, 0x00},
			{0x0103, 0x01},
			{0x3034, 0x1a},
			{0x3035, 0x21},
...
//			 --- 밝기 증가용 (Exposure 증가) ---
    ...
    config_word_t const cfg_vga_90[] = {
    		{0x0100, 0x00},
			{0x0103, 0x01},
			{0x3035, 0x11},
			{0x3036, 0x46},
    ...
    // --- AWB ---
    config_word_t const cfg_awb_simple[] = {
        {0x3406, 0x00} // AWB enable (default)
    };
    config_word_t const cfg_awb_disabled[] = {
        {0x3406, 0x01} // AWB disable
    };

    config_modes_t const modes[] = {
        { MAP_ENUM_TO_CFG(MODE_2592_1944_15, cfg_2592_1944_15) },
        { MAP_ENUM_TO_CFG(MODE_1080P_1920_1080_30, cfg_1080p_30) },
        { MAP_ENUM_TO_CFG(MODE_VGA_640_480_60, cfg_vga_90) }
    };
    config_awb_t const awbs[] = {
        { MAP_ENUM_TO_CFG(AWB_DISABLED, cfg_awb_disabled) },
        { MAP_ENUM_TO_CFG(AWB_SIMPLE, cfg_awb_simple) }
    };
} // namespace OV5647_cfg
...
public:
    ...
    {
        uint8_t id_h, id_l;
        readReg(0x300A, id_h); // ID high: 0x56
        readReg(0x300B, id_l); // ID low:  0x47
        if (id_h != 0x56 || id_l != 0x47)
            throw HardwareError(HardwareError::WRONG_ID, "OV5647 wrong ID detected");
        // 소프트 리셋
        writeReg(OV5647_cfg::REG_SW_RESET, 0x01);
        usleep(50000);
    }
    ...
    {
        if (mode >= OV5647_cfg::MODE_END) return ERR_LOGICAL;
    ...
    {
        if (awb >= OV5647_cfg::AWB_END) return ERR_LOGICAL;
        auto cfg_awb = &OV5647_cfg::awbs[awb];
        writeConfig(cfg_awb->cfg, cfg_awb->cfg_size);
        return OK;
    }
    ...
    {
        switch(test) {
        case OV5647_cfg::TEST_DISABLED:
            writeReg(OV5647_cfg::OV5647_REG_COLORBAR, 0x00); break;
        case OV5647_cfg::TEST_COLORBAR:
            writeReg(OV5647_cfg::OV5647_REG_COLORBAR, 0x80); break;
        default: break;
        }
    }
...
private:
    void usleep(uint32_t time) { for (volatile uint32_t i=0; i<time*8; ++i); }
    ...
    I2C_Client& iic_;
    GPIO_Client& gpio_;
    uint8_t dev_address_ = (0x6C >> 1); // OV5647 I2C Address (0x36)
