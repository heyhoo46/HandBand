module rgb_color_detect #(
    parameter  ABS_TH   = 128, // 48
    parameter  MARGIN_B = 64,   // 16
    parameter  MARGIN_G = 64,   // 16
    ...
    wire raw_blue_rel = (B > R + MARGIN_B) && (B > G + MARGIN_G);
    wire raw_blue_abs = (B > ABS_TH);
    wire raw_blue     = raw_blue_rel && raw_blue_abs;

    wire raw_red_rel  = (R > G + MARGIN_G) && (R > B + MARGIN_B);
...
//     reg [$clog2(IMG_WIDTH)-1:0] x_pixel_cnt;
//     reg [$clog2(IMG_HEIGHT)-1:0] y_pixel_cnt;
...
//     wire prev_blue = line_buf_blue[x_pixel_cnt];
//     wire prev_red = line_buf_red[y_pixel_cnt];
//     reg [HIST_LEN-1:0] hist_blue, hist_red;
...
//             else if (tlast) begin
//                 x_pixel_cnt <= 0;
//                 y_pixel_cnt <= (y_pixel_cnt == IMG_HEIGHT-1) ? 0: y_pixel_cnt + 1;
//             end else begin
...
//             line_buf_blue[x_pixel_cnt] <= raw_blue;
//             line_buf_red[y_pixel_cnt]  <= raw_red;
