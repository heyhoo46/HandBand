...
void Presenter_Execute(void) {
	...
	// ── 1. 데이터 채우기 ──
	vectorData.angle_deg = rand() % 360;
	vectorData.magnitude = rand() % 100;

	// 실제 모델에서 읽어올 때는 위 두 줄을 지우고 아래를 사용
	// CP_Model_PopPolar(&vectorData);

	// ── 2. 인덱스 계산 ──
	uint8_t angIndex = AngletoIndexNum(vectorData.angle_deg);
	uint8_t magIndex = MagnitudeToIndexNum(vectorData.magnitude);

	// ── 3. Effect 계산 ──
	e_stageEffect effect = Presenter_Effect(vectorData.angle_deg,
			vectorData.magnitude);

	// ── 4. 디버그 출력 ──
	char str[160];
	int len = sprintf(str,
			"TEST: angle=%3u(idx=%u), mag=%3u(idx=%u), Effect=0x%04X\r\n",
			vectorData.angle_deg, angIndex, vectorData.magnitude, magIndex,
			(uint16_t) effect);
